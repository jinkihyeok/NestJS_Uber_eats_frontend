...
export const Restaurant = () => {
  ...
  const isOptionSeleted = (dishId: number, optionName: string) => {
  ...
  return (
    <div>
      ...
      <div className="container pb-22 flex flex-col items-end mt-20">
        {!orderStarted && (
            ...
            >
              {dish.options?.map((option, index) => (
                <DishOption
                  key={index}
                  dishId={dish.id}
                  isSelected={isOptionSeleted(dish.id, option.name)}
                  name={option.name}
                  extra={option.extra}
